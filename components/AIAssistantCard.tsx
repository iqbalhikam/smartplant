...
export default function AIAssistantCard({ telemetry }: AIAssistantCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-slate-900/40 border border-slate-800/80 backdrop-blur-xl rounded-2xl p-6 shadow-xl hover:border-slate-700/80 transition-all duration-300 overflow-hidden"
    >
      <div className="flex items-center justify-between border-b border-slate-800/60 pb-4 mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-bold text-slate-200 tracking-wide text-sm">Asisten AI Botanist</h3>
            <p className="text-[10px] text-slate-400">Analisis kondisi tanaman secara pintar oleh Google Gemini</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${aiEnabled ? "bg-indigo-500" : "bg-slate-800"
              }`}
          >
            <span
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                className="px-5 py-2.5 bg-linier-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white disabled:opacity-50 rounded-xl text-xs font-bold tracking-wide transition-all duration-300 flex items-center gap-2 active:scale-95 cursor-pointer shadow-lg shadow-indigo-500/10"
              >
                {aiLoading ? (
                  <>
            ...
            <AnimatePresence>
              {aiLoading && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className="p-4 bg-slate-950/40 border border-slate-800/60 rounded-xl flex items-center gap-3"
                >
              ...
              {aiResult && !aiLoading && (
                <motion.div
                  ...
                  className="p-4 bg-linier-to-r from-indigo-950/20 to-purple-950/20 border border-indigo-500/20 rounded-xl relative overflow-hidden"
                >
                  <div className="absolute -right-12 -bottom-12 w-28 h-28 bg-indigo-500/5 rounded-full blur-xl pointer-events-none" />
                  ...
                  <div className="flex gap-3 relative z-10">
                    ...
                    <div className="space-y-1.5 flex-1">
                      <div className="text-[10px] font-bold text-indigo-400 tracking-wider uppercase">Rekomendasi Ahli Botani AI</div>
                      <blockquote className="text-xs text-slate-200 leading-relaxed font-medium">
                        "{aiResult}"
                      </blockquote>
                    </div>
                  </div>
