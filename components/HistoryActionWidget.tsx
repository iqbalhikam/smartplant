...
const ActionChart = dynamic(() => import("./HistoryCharts").then(mod => mod.ActionChart), { ssr: false, loading: () => <div className="h-64 w-full flex justify-center items-center"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div> });
...
export default function HistoryActionWidget({ isLoading, actionLogs, chartData, variant }: { isLoading: boolean, actionLogs: ActionLog[], chartData: any, variant?: WidgetVariant }) {
  const actionContent = (
    <>
                ...
                <tbody>
                  {actionLogs.map((log) => (
                    <tr key={log.id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="px-4 py-3"><span className="px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded font-mono text-xs text-primary dark:text-secondary">{log.kodeRule}</span></td>
                      <td className="px-4 py-3">{log.durasiPompaMs > 0 ? `${log.durasiPompaMs}ms` : 'Otomatis'}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${log.lampuNyala ? 'bg-warning/10 text-warning' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
