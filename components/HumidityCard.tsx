...
export default function HumidityCard({ telemetry, variant = "default" }: HumidityCardProps) {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          {isAvailable ? <Cloud className="w-4 h-4 text-primary" /> : <PowerOff className="w-4 h-4 text-slate-400" />}
          <h3 className="font-semibold text-text-secondary text-xs uppercase tracking-wider">Kelembapan</h3>
        </div>
  ...
  if (variant === "glassmorphism") {
    return (
      <motion.div
        variants={itemVariants}
        className={`bg-surface backdrop-blur-xl border border-white/40 dark:border-white/10 rounded-3xl shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] flex flex-col h-full overflow-hidden p-6 transition-all ${!isAvailable ? 'opacity-60 grayscale' : 'hover:shadow-[0_8px_32px_0_rgba(59,130,246,0.15)]'}`}
      >
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-white/20 pb-3 mb-3 shrink-0 relative z-10">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold tracking-wide text-sm text-white drop-shadow-sm">Kelembapan</h3>
          </div>
        </div>
        <div className="flex-1 flex flex-col justify-center items-center relative z-10">
          <div className="flex items-start drop-shadow-md">
            ...
            <span className="text-xl font-bold mt-1 text-white/80">%</span>
          </div>
        </div>
        <div className="flex items-center justify-center text-[10px] font-medium mt-3 pt-3 border-t border-white/20 shrink-0 relative z-10">
          {isAvailable ? (
            <span className="text-white/90 flex items-center gap-1">
              <CloudRain className="w-3.5 h-3.5 text-white" /> Udara Normal
            </span>
          ) : (
      ...
      <motion.div variants={itemVariants} className={`bg-background border border-primary/50 rounded-xl p-5 flex flex-col h-full relative overflow-hidden shadow-[0_0_15px_rgba(59,130,246,0.3),inset_0_0_20px_rgba(59,130,246,0.1)] ${!isAvailable ? 'opacity-60 grayscale' : ''}`}>
        ...
        <div className="flex justify-between items-start mb-6">
           <div className="flex items-center gap-2">
             ...
             <h3 className="font-mono text-secondary uppercase tracking-[0.2em] text-xs drop-shadow-[0_0_2px_rgba(59,130,246,0.5)]">SYS_HUMIDITY</h3>
           </div>
        </div>
        <div className="flex-1 flex items-center justify-center relative">
           ...
           <div className="relative flex items-end">
             <span className="text-6xl font-mono text-secondary drop-shadow-[0_0_10px_rgba(96,165,250,0.8)] leading-none">
               {isAvailable ? humidity.toFixed(1) : "ERR"}
             </span>
  ...
  if (variant === "neobrutalism") {
    return (
      <motion.div variants={itemVariants} className={`border-4 border-black rounded-xl p-5 flex flex-col h-full shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden transition-all hover:-translate-y-1 hover:shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] bg-[#38bdf8] ${!isAvailable ? 'opacity-60 grayscale' : ''}`}>
        <div className="flex justify-between items-start mb-6 border-b-4 border-black pb-4">
           <div className="flex items-center gap-2">
             <div className="bg-white border-2 border-black p-1 rounded-md shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
               <Cloud className="w-5 h-5 text-black" fill="black" />
             </div>
             <h3 className="font-black text-black uppercase tracking-widest text-sm">KELEMBAPAN</h3>
           </div>
        </div>
    ...
    return (
      <motion.div variants={itemVariants} className={`bg-[#e0e5ec] dark:bg-slate-800 rounded-3xl p-6 flex flex-col h-full shadow-[9px_9px_16px_rgb(163,177,198,0.6),-9px_-9px_16px_rgba(255,255,255,0.5)] dark:shadow-[8px_8px_16px_rgba(0,0,0,0.4),-8px_-8px_16px_rgba(255,255,255,0.05)] border-none ${!isAvailable ? 'opacity-60 grayscale' : ''}`}>
        <div className="flex justify-between items-center mb-6">
           ...
           <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Kelembapan</span>
        </div>
        <div className="flex-1 flex flex-col justify-center items-center">
           <div className="w-28 h-28 rounded-full flex items-center justify-center bg-[#e0e5ec] dark:bg-slate-800 shadow-[6px_6px_12px_rgba(163,177,198,0.6),-6px_-6px_12px_rgba(255,255,255,0.5)] dark:shadow-[6px_6px_12px_rgba(0,0,0,0.4),-6px_-6px_12px_rgba(255,255,255,0.05)] mb-4 border-4 border-[#e0e5ec] dark:border-slate-800">
             <span className="text-3xl font-black text-slate-700 dark:text-slate-200">{isAvailable ? `${humidity.toFixed(1)}` : '--'}</span>
           </div>
        </div>
      </motion.div>
    );
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700/50 pb-2 mb-2 shrink-0">
        <div className="flex items-center gap-2">
          {isAvailable ? (
            <div className="p-1.5 bg-blue-50 dark:bg-primary/10 rounded-lg border border-blue-100 dark:border-primary/20">
              <Cloud className="w-4 h-4 text-primary dark:text-secondary" />
            </div>
          ) : (
          ...
          <h3 className="font-bold text-text-primary tracking-wide text-xs">Kelembapan</h3>
        </div>
      </div>
